...
class CodeAnalyzer:
    ...
    def _collect_files(self):
        ...
        print(f"[OK] Found {len(self.results['files'])} Python file(s)")
    ...
    def _run_flake8(self):
        ...
        import sys
        try:
            # On lance Flake8 via le moteur Python actuel
            result = subprocess.run(
                [sys.executable, "-m", "flake8", str(self.path)],
                capture_output=True,
                text=True,
                check=False
            )
            ...
            for line in result.stdout.splitlines():
                ...
                # rsplit(":", 3) coupe les 3 derniers ":" en partant de la droite.
                # Cela protège le "C:" au début du chemin.
                parts = line.rsplit(":", 3)
                ...
                if len(parts) >= 4:
                    ...
                    try:
                        # On récupère le code d'erreur (ex: F401)
                        message_clean = message.strip()
                        code = message_clean.split()[0] if message_clean else "???"

                        self.results["flake8_issues"].append({
                            "file": filepath.strip(),
                            "line": int(lineno.strip()), # Plus d'erreur ici !
                            "column": int(col.strip()),
                            "code": code,
                            "message": message_clean
                        })
                    except (ValueError, IndexError):
                        # En cas de ligne malformée, on passe à la suivante
                        continue
    ...
    def _run_pylint(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pylint", str(self.path), "--output-format=json"],
                capture_output=True,
                text=True
            )
    ...
    def _run_radon(self):
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "radon", "cc", str(self.path), "-j"],
                capture_output=True,
                text=True
            )
