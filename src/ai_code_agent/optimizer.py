...
class CodeOptimizer:
    ...
    def _check_tools_installed(self):
        ...
        for tool in required:
            try:
                # On demande au Python actuel s'il peut lancer le module
                subprocess.run(
        ...
        if missing:
            ...
            print(" [OK] All optimization tools (black, isort) are ready")
    ...
    def _run_isort(self, files: List[str]):
        ...
        for filepath in files:
            try:
                subprocess.run([sys.executable, "-m","isort", filepath], check=True, capture_output=True)
    ...
    def _run_black(self, files: List[str]):
        ...
        for filepath in files:
            try:
                subprocess.run([sys.executable, "-m","black", "--quiet", filepath], check=True, capture_output=True)
